...
(define-command-input-handler %cmd-paste-buffer-arg (session args)
  ...
  (let* ((name       (%buffer-name-from-flags flags))
         ...
         ;; Canonical scriptable default: LF -> CR so a multi-line paste
         ;; submits each line; -s overrides the replacement, -r keeps the raw
         ;; bytes.
...
(define-command-input-handler %cmd-save-buffer-arg (session args)
...
(define-command-input-handler %cmd-load-buffer-arg (session args)
