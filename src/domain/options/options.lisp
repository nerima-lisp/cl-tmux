...
;;;
;;; define-option-table encapsulates the three-phase expand pattern shared by
;;; define-tmux-options and define-server-options: registering spec metadata,
;;; initialising runtime default values, and populating the immutable
;;; known-registry fallback set-option -u reads from.
;;;
;;; Parameters:
...
;;;   KNOWN-REGISTRY-VAR — the *known-*-option-registry* immutable fallback
;;;   SPECS              — list of (name type default) triples

(defmacro define-option-table (registry-var storage-var known-registry-var &rest specs)
  "Register option specs in REGISTRY-VAR, initialise STORAGE-VAR with defaults, and
   populate KNOWN-REGISTRY-VAR (the immutable fallback set-option -u falls back to)
   with the same spec metadata.  Each SPEC has the form (name type default) where
   TYPE is :boolean, :integer, or :string."
  `(progn
     ...
     ,@(mapcar (lambda (spec)
               ...
               specs)
     ;; Phase 3: populate the immutable known-registry fallback
     ,@(mapcar (lambda (spec)
                 (destructuring-bind (name type default) spec
                   `(setf (gethash ,name ,known-registry-var)
...
(defmacro define-tmux-options (&rest specs)
  ...
  `(define-option-table *option-registry* *global-options* *known-option-registry*
     ,@specs))
...
(defmacro define-server-options (&rest specs)
  ...
  `(define-option-table *server-option-registry* *server-options* *known-server-option-registry*
     ,@specs))
