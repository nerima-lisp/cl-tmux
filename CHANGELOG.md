...
- **Raised test abstraction: 15 `dolist`-over-a-case-table `it`s across 6
  files now use cl-weave's `it-each`**, so 77 cases run and report as
  independently-named tests instead of hiding inside opaque blocks — the
  `desc` field each case already carried, previously `(declare (ignore
  desc))`'d as dead weight, now names the generated test.
